...
class HiddenStructureLearning:
    def __init__(
        self, events_df, Model_d, Model_o, n_pl, n_roles, args, libmode, tol=1e-1, defend_iter=100, offend_iter=100
    ):
    ...
    def train_hmm(self, player_inds, n_iter, random_state=42, verbose=True):
        print(
            "Training for {0} players and {1} player_roles with iterations: {2}".format(
                len(player_inds), self.n_roles, n_iter
            )
        )
        n_pl = self.n_pl
        if n_pl == 5:
    ...
    def visualize_HMM(self, model):
        if (self.defend_iter > 10 and self.team == 0) or (self.offend_iter > 10 and self.team == 1):
            ...
            try:
                plt.savefig("figure/HMM/" + data + "_Gaussian_HMM_" + XO + ".png", bbox_inches="tight")
            except:
                import pdb
    ...
    def predict_hmm(self, trainModel, player_inds, n_iter, random_state=42, verbose=True):
        ...
        return {"X": X, "lengths": lengths, "model": trainModel, "cmeans": cmeans}
    ...
    def assign_roles(self, trainModel, player_inds, n_iter, mode="euclidean"):
        ...
        for i in range(n):
            ...
            try:
                role_assignments[i, :] = np.array(self.assign_ind(cost))
            except:
                import pdb
    ...
    def reorder_moment(self):
        ...
        if self.offend_iter > 0:
            ...
            defend_role_assignments, defend_result = self.assign_roles(
                self.Model_d, player_inds=self.defend_players, n_iter=self.defend_iter
            )
            self.team = 1
            offend_role_assignments, offend_result = self.assign_roles(
                self.Model_o, player_inds=self.offend_players, n_iter=self.offend_iter
            )
            ...
            reordered_defend = copy.deepcopy(
                reorder_moment_(self.defend_players, original, reordered, defend_role_assignments, 0)
            )
            reordered_all = copy.deepcopy(
                reorder_moment_(self.offend_players, original, reordered_defend, offend_role_assignments, self.n_pl)
            )
    ...
    def plotPosition3(self, team_A, team_B, ball, K, i, markersize, TextCircle, allPlayers):
        ...
        if allPlayers:
            if TextCircle == 1:
                ...
                ax.add_patch(im_ball)
                im_ball = plt.scatter(
                    ball[i, 0], ball[i, 1], marker=".", s=markersize, fc="orange", ec="k"
                )  # for legend only
        ...
        for tm in range(Tm):
            ...
            for j in range(start_pl, n_team_agent):
                ...
                if TextCircle == 1:  # player circle
                    ...
                    if tm == 1:  # offense
                        im_team[tm] = patches.Circle((xx, yy), radius=markersize - 0.15, fc=clr, ec="k")
                    else:
                        im_team[tm] = patches.CirclePolygon(
                            (xx, yy), radius=markersize, resolution=3, fc=clr, ec="k"
                        )  # triangle
