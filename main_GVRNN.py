...
if __name__ == "__main__":
    ...
    os.makedirs(game_file0, exist_ok=True)
    ...
    if os.path.isfile(game_files + "_te_0.pkl"):
        ...
        if create_Train:
            if args.attack_defend:
                X_train_all, Y_train_all, I_train_all = get_sequences_attack(
                    game_data,
                    activeRoleInd,
                    totalTimeSteps + 5,
                    overlapWindow,
                ...
                X_train_all, Y_train_all = get_sequences(
                    game_data,
                    activeRoleInd,
                    totalTimeSteps + 5,
                    overlapWindow,
        ...
        if args.attack_defend:
            ...
            X_test_all, Y_test_all, I_test_all = get_sequences_attack(
                game_data_te,
                activeRoleInd,
                totalTimeSteps + 5,
                overlapWindow,
            ...
            X_test_all, Y_test_all = get_sequences(
                game_data_te,
                activeRoleInd,
                totalTimeSteps + 5,
                overlapWindow,
        ...
        for e in range(args.n_epoch):
            ...
            if not args.finetune:
                ...
                optimizer = torch.optim.SGD(
                    filter(lambda p: p.requires_grad, model.parameters()), lr=5e-4, momentum=0.9
                )
    ...
    state_dict = torch.load(
        "{}_best.pth".format(init_pthname, params["model"]), map_location=lambda storage, loc: storage
    )
    model.load_state_dict(state_dict)
    ...
    if True:
        ...
        for batch_idx, (data, macro_intents, ind_player) in enumerate(loader):
            ...
            try:
                for i in range(n_sample):
                    samples_GT[:, :, batch_idx * batchSize_test : (batch_idx + 1) * batchSize_test] = (
        ...
        print("test sample")
        # Sample trajectory
        samples = [np.zeros((args.horizon + 1, args.n_agents, len_seqs_test, featurelen)) for t in range(n_sample)]
        hard_att = 0
        ...
        for r in range(rep_smp):
            ...
            for i in range(args.n_agents):
                try:
                    ...
                    tmp_data = (
                        samples_GT[:, :, :, n_feat * i : n_feat * i + n_feat]
                        - samples[r][:, :, :, n_feat * i : n_feat * i + n_feat]
                    )
        ...
        try:
            pickle.dump(
                [samples, samples_GT, hard_att, ind_players, losses2, macros, losses_t],
