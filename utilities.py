...
class PlotGame:
    ...
    def load_moment2img(self, data, event_number, moment_number, return_img=False):
        ...
        player_xy = np.array(
            [np.array(x[5][1:])[:, :4] for x in data["events"][ii]["moments"]]
        )  # create matrix of player data
        ...
        clock_info = ax.annotate(
            "",
            xy=[94.0 / 2 - 6.0 / 1.5 + 0.1, 50 - 6.0 / 1.5 - 0.35],
            color="black",
        ...
        clock_test = "Quarter {:d}\n {:02d}:{:02d}\n {:03.1f}".format(
            quarter, int(game_clock) % 3600 // 60, int(game_clock) % 60, game_shot
        )
        clock_info.set_text(clock_test)

        # the event title
        temp = (
            str(data["events"][ii]["home"]["abbreviation"])
            + " vs. "
            + str(data["events"][ii]["visitor"]["abbreviation"])
            + "\n"
        )
        for idx, pp in playbyplay.iterrows():
            temp = (
                temp
                + str(pp["HOMEDESCRIPTION"])
                + " , "
        ...
        for kk in range(player.shape[0]):  # create circle object and text object for each player
            ...
            # player circle
            player_circ = plt.Circle((xx, yy), 2.2, facecolor=color_dict[team_id][0], edgecolor="k")
        ...
        plt.plot(
            team_1_xy_mean[: jj + 1, 0], team_1_xy_mean[: jj + 1, 1], "o", color=color_dict[team_ids[0]][0], alpha=0.2
        )
        ...
        plt.plot(
            team_2_xy_mean[: jj + 1, 0], team_2_xy_mean[: jj + 1, 1], "o", color=color_dict[team_ids[1]][0], alpha=0.2
        )
